...
savePersonne(id: any, personne: Personne) {
  ...
  if (!personne.nom || !personne.prenom || !personne.adresse || !personne.telephone|| !personne.email) {
...
if (index === -1) {
  console.error('Personne non trouvée dans la liste.');
  return;
}
...
this._personneService.updatePersonne(id, personne).subscribe({
  next: (updatedPersonne) => { // Si la mise à jour est réussie, on obtient les données mises à jour
    this.listePersonne[index] = updatedPersonne; // On met à jour la personne dans la liste locale avec les données reçues
    ...
    this._messageService.add({
      severity: 'success',
      summary: 'Succès',
      detail: 'Patient modifié avec succès',
      life: 5000, // La durée pendant laquelle le message doit être affiché (en millisecondes)
    });
