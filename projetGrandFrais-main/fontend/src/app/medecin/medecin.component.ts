...
saveMedecin(id: any, medecin: Medecin) {
  this.submitted = true; // On indique que le formulaire a été soumis

  // Vérification des champs du formulaire
  if (!medecin.nom || !medecin.specialite|| !medecin.email ) {
  ...
  if (this.btnText === 'Ajouter') { // Si on est en train d'ajouter une nouvelle personne
    ...
    this._medecinService.createMedecin(medecin).subscribe({
    ...
    const index = this.listeMedecin.findIndex((p) => p.id === id); // On récupère l'index de la personne dans la liste

// Vérification si l'index est valide
if (index === -1) {
  console.error('medecin non trouvée dans la liste.');
  return;
}

// On appelle le service pour mettre à jour la personne avec l'identifiant et les données fournies
this._medecinService.updatMedecin(id, medecin).subscribe({
  next: (updatedPersonne) => { // Si la mise à jour est réussie, on obtient les données mises à jour
    this.listeMedecin[index] = updatedPersonne; // On met à jour la personne dans la liste locale avec les données reçues

    // On affiche un message de succès avec l'objet MessageService
    this._messageService.add({
      severity: 'success',
      summary: 'Succès',
      detail: 'Medecin modifié avec succès',
      life: 5000, // La durée pendant laquelle le message doit être affiché (en millisecondes)
    });
  ...
  error: (error: any) => { // Si une erreur se produit pendant la mise à jour
    ...
    this._messageService.add({
      severity: 'error',
      summary: 'Erreur',
      detail: 'La mise à jour du medecin a échoué',
      life: 5000, // La durée pendant laquelle le message doit être affiché (en millisecondes)
    });
  },
  complete: () => {
 ...
 deleteMedecin(id: number) {
  ...
  this._confirmationService.confirm({
    message: 'Etes vous sûr de supprimer le patient selectionné ?', // Le message à afficher dans la boîte de confirmation
    ...
    accept: () => { // Si l'utilisateur accepte la confirmation
      ...
      this._medecinService.deleteMedecin(id).subscribe({
